...
class HOAuthAction extends CAction
{
	...
	protected $_fullnameAttribute = false;
	protected $_contact_numberAttribute=false;
	protected $_nricAttribute=false;
	protected $_addressAttribute=false;
	/**
	...
	public function run()
	{
		...
		if($this->enabled)
		{
			...
			if($this->useYiiUser || file_exists(Yii::getPathOfAlias('application.modules.user.components') . '/UWrelBelongsTo.php'))
			{
				...
				$this->usernameAttribute = 'fullname';
				$this->_emailAttribute = 'email';
				$this->_fullnameAttribute = 'fullname';
				$this->_contact_numberAttribute='contact_number';
				$this->_nricAttribute='nric';
				$this->_addressAttribute='address';
			}
			else
			{
	...
	protected function processUser($user, $userProfile)
	{
		...
		$form = new HUserInfoForm($user, $this->_emailAttribute, $this->usernameAttribute, $this->_fullnameAttribute , $this->_nricAttribute , $this->_contact_numberAttribute , $this->_addressAttribute);
...
//				UserModule::sendMail($user->email,UserModule::t("You registered on {site_name}",array('{site_name}'=>Yii::app()->name)),UserModule::t("To activate your account, please go to {activation_url}",array('{activation_url}'=>$activation_url)));
				UserModule::sendPHPMailer($user->email,UserModule::t("You registered on {site_name}",array('{site_name}'=>Yii::app()->name)),UserModule::t("To activate your account, please go to {activation_url}",array('{activation_url}'=>$activation_url)));
			}
		}
		else
		{
