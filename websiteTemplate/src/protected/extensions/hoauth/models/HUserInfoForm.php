...
class HUserInfoForm extends CFormModel {
	...
	public $fullnameAtt;
	public $nricAtt;
	public $contact_numberAtt;
	public $addressAtt;

	public function rules()
	{
		return array(
			array('fullname, contact_number, nric, address','required','message'=>'Please enter your {attribute}'),
	 ...
	 * @access public
	 */
	public function __construct($model, $emailAtt, $nameAtt = null, $fullnameAtt, $nricAtt, $contact_numberAtt, $addressAtt, $scenario = 'both')
	{
		if(empty($emailAtt))
		...
		if(empty($nameAtt)){
			...
			$this->fullname = $model->$nameAtt;
		}
		if (empty($fullnameAtt)) {
			$this->fullname = $model->$nameAtt;
		}
		if (empty($contact_numberAtt)) {
			$this->contact_number = $model->$contact_numberAtt;
		}
		if (empty($nricAtt)) {
			$this->ic_number = $model->$nricAtt;
		}
		if (empty($addressAtt)) {
			$this->address = $model->$addressAtt;
		}
		...
		$this->nric=$model->nric;
		$this->address=$model->address;

		$this->nameAtt = $nameAtt;
		$this->emailAtt = $emailAtt;
		$this->fullnameAtt = $fullnameAtt;
		$this->nricAtt = $nricAtt;
		$this->contact_numberAtt = $contact_numberAtt;
		$this->addressAtt = $addressAtt;
		$this->_model = $model;
	...
	public function attributeLabels()
	{
		return array(
			...
			'fullname'=>$this->_model->getAttributeLabel($this->fullnameAtt),
			'nric'=>$this->_model->getAttributeLabel($this->nricAtt),
			'contact_number'=>$this->_model->getAttributeLabel($this->contact_numberAtt),
			'address'=>$this->_model->getAttributeLabel($this->addressAtt),
	...
	public function getValidUserModel()
	{
		...
		if($this->_model->isNewRecord && strpos($this->scenario, '_pass') === false)
		{
			$this->_model->setAttributes(array(
				...
				$this->fullnameAtt => $this->fullname,
				$this->nricAtt => $this->nric,
				$this->contact_numberAtt => $this->contact_number,
				$this->addressAtt => $this->address,
				), false);
